...
const Form: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.post("http://localhost:5001/api/auth/expenses", requestData, {
        headers: {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-800 p-7">
      <div className="w-full max-w-2xl bg-[#121212] shadow-xl rounded-lg p-8">
        <h1 className="text-3xl font-bold text-center text-gray-300 mb-6">
          Add a Transaction
        </h1>

        <p className="text-gray-400 text-center mb-6">
          Keep track of your expenses and income with ease.
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            <select
              ...
              onChange={handleChange}
              required
              className="w-full px-3 py-3 bg-[#1e1e1e] text-gray-200 border border-gray-600 rounded-lg focus:ring-2 focus:ring-[#00aaff] focus:outline-none"
            >
              <option value="">Select Type</option>
              <option value="Income">Income</option>
              <option value="Expense">Expense</option>
          ...
          <button
            type="submit"
            className="w-full bg-indigo-600 text-white font-bold py-3 rounded-lg hover:bg-indigo-700 transition delay-150 duration-300 ease-in-out hover:-translate-y-1"
          >
            Add Transaction
          </button>
        </form>
